...
function handleCountry( currentCountry) {
    if (currentCountry < countriesArray.length) {
...
function produceCountryMessage(country) {
    ...
    var countryKM = new KeyedMessage(country.code, JSON.stringify(country));
    payloads.push({ topic: countriesTopic, messages: countryKM, partition: 0 });
